...
public class DriveTrain extends SubsystemBase {
    ...
    private final MotorController rightMotor = new PWMSparkMax(
        RIGHT_MOTOR_PORT
    );

    private final DifferentialDrive driver = new DifferentialDrive(
        leftMotor,
        rightMotor
    );
    ...
    public Command driveCommand(DoubleSupplier fnX, DoubleSupplier fnY) {
        return new RunCommand(() ->
            this.drive(fnX.getAsDouble(), fnY.getAsDouble())
        );
    }
